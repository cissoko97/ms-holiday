        ...
        try {
            if ("password".equalsIgnoreCase(request.getGrantType())) {
                try {
                    ...
                    throw new CustomException("failed to authenticate user");
                }
                ...
                if (request.getRefreshToken() == null) {
                    ...
                    throw new CustomException("Try to register by token with null token");
                }
                Jwt jwtDecode = null;
                try {
                    jwtDecode = jwtDecoder.decode(request.getRefreshToken());
                } catch (Exception e) {
                    throw new CustomException("failed to decode the token");
                }
